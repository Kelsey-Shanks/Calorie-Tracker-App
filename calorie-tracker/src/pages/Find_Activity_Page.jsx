...
function Find_Activity_Page ( {setExerciseToUse} ) {   
    ...
    const [all_exercises, setAllExercises] = useState([]);                      // sets type for query
    const navigate = useNavigate();

    const name_info = () => {                                                           // prompt function
        const message1 = `Search for an existing activity by typing the name of the activity.\n`;
        const message2 = "The existing options will be displayed below the search bar.\n";
        const message3 = `You must create a new activity if the intended one is not displayed.`
    ...
    const getExercises = async () => {
        const response = await fetch("/selections?type=exercise");     // get all meal options
    ...
    let input_handler_2 = async (e) => {  // try the microservice
        ...
        const response = await fetch (
            `/search_service?type=exercise&search=${input_search}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json'}, 
                body: JSON.stringify(all_exercises),
            }
        )
                                ...
                                <li key={i}>
                                    <img src={result.image} width="50" height="50"/> 
                                        <button id="find-options-btns" onClick={e => {    // button selects entry
                                            e.preventDefault();                         // prevent default behavior
                                            setExerciseToUse(result);                       // function selects JSON object (dispalyed name) 
...
export default Find_Activity_Page;